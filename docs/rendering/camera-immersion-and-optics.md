...
- **External Camera Design Patterns**: The MSFS 2024 SDK and Cinemachine 3.1 manuals expose velocity-based external framing, independent damping, lookahead, and camera-radius obstacle avoidance as configurable techniques [2, 9–12]. These are implementation examples, not evidence of vestibular safety or validated tuning for this game.
...
The render loop derives air-relative acceleration from adjacent fixed 144 Hz simulation samples, interpolates that signal with the rendered pose, and passes it to the camera. A 6 Hz one-pole low-pass also protects the inertia from step and wind noise. Callers without fixed-step acceleration use a filtered velocity derivative. The critically damped offset runs in camera-local right, up, and forward coordinates, with targets bounded to $\pm0.22$ m laterally, $\pm0.18$ m vertically, and $\pm0.12$ m longitudinally.
...
The render loop passes its cached world collision-height query to the camera. The camera sweeps its nominal $14.5$ m back / $3.6$ m up boom in 1 m increments, checking the center and eight points around a 0.55 m radius. When the aircraft's projected rear bound requires more than the nominal back leg, the camera extends that leg to keep the model beyond the near plane. This check uses the actual look direction, including target elevation and predictive lead. The collision sweep covers the extended boom. At the first terrain, roof, tree-crown, or scatter-height obstruction, five binary-search steps refine the last-clear/first-blocked interval and leave a 5 cm retreat margin. It returns toward full length with a 3 s$^{-1}$ exponential release.

This is a bounded height-field approximation of a camera-sphere sweep, not a triangle-level capsule cast: `WorldNeighborhood::collision_height_at` provides a conservative vertical envelope for terrain and known objects. The extended boom uses at most 32 coarse stations; near the current airframe envelope it remains close to the nominal 1 m spacing. Refinement adds at most five more stations. Thin obstacles can still be missed between coarse stations, and broad height envelopes can over-contract the boom. The aircraft's conservative local AABB sets a projected near-plane floor at `NEAR` plus 0.25 m along the actual view direction; its bounds include the 0.55 m camera sweep radius and padding for expected wing flex/control motion. The nominal aft leg extends when needed, and collision contraction is clamped to that safe position. Keep the bounds in `camera.rs` synchronized with the procedural geometry in `crates/airframe/src/airframe.rs`, node transforms in `engine/src/anim.rs`, and wing flex in `engine/shaders/plane.mesh`. Near obstacles may therefore stop boom contraction farther out to preserve aircraft framing. The CSV trace records both boom fraction and axial setback.
...
- **Unit Tests**:
  ...
  - `sim::camera::tests::velocity_derivative_noise_is_low_passed_across_variable_render_steps`
  - `sim::camera::tests::fixed_step_acceleration_takes_priority_over_render_velocity_differences`
  - `sim::camera::tests::swept_camera_boom_contracts_at_a_ridge_and_returns_slowly`
  - `sim::camera::tests::swept_camera_boom_refines_contact_below_the_coarse_sample_spacing`
  - `sim::camera::tests::near_plane_airframe_bounds_cover_current_generated_geometry`
  - `sim::camera::tests::swept_boom_keeps_the_airframe_bounds_beyond_the_near_plane`
  - `sim::camera::tests::swept_boom_extends_when_oblique_airframe_bounds_exceed_nominal_depth`
  - `sim::camera::tests::compound_roll_pitch_reversal_sideslip_and_ridge_sequence_stays_continuous`
  - `sim::camera::tests::world_up_projects_toward_top_of_vulkan_image`
  - `engine::plane::tests::test_ubo_tail_and_bytes_alignment`
- **Deterministic tuning trace**: `cargo run -p sim --example camera_trace --locked > camera-trace.csv` emits a fixed-step cruise, hard-bank, barrel-roll, loop pitch-reversal, high-AoA, and low-altitude synthetic ridge sequence. The CSV includes airflow blend, local pitch/yaw/roll error, translational offset, boom fraction and axial setback, FOV, speed, altitude, AoA, and sideslip. It validates control signals but is not rendered visual acceptance or a record from a live flight session.
- **Native target-GPU capture (2026-09-23)**: One 10,000-present `dist` run from commit `4e0312b` (before the near-plane boom floor) used the NVIDIA GeForce RTX 4060 Laptop GPU, driver 580.173.02, on the 2880×1800/120 Hz workstation display. The maximized window and scene were 2880×1646; quality was Cinematic, one complete scene per present, 1× MSAA, ray-traced shadows and mesh shaders enabled, MAILBOX present mode. The recorded workload was the default spawn in calm wind; boost and bank debug overrides were inactive. The near-plane floor changes camera behavior during obstruction and has not been remeasured on the target GPU.
  - Submitted-present intervals: mean 6594.224 μs (151.648/s), p50 6462.549 μs, p95 9111.444 μs, p99 13402.396 μs, max 38491.519 μs; 9947/10000 exceeded 1 ms. The submission target failed.
  - GPU frame timestamps: mean 6411.651 μs, p50 6405.792 μs, p95 7638.208 μs, p99 8326.208 μs, max 9157.728 μs. The logged CPU `sim+camera` stage averaged 330.0 μs; this combines simulation and camera work and does not isolate the sweep cost.
...
- **Acceptance status**: `cargo test --workspace --locked` and `python3 tools/check_shaders.py` passed locally on 2026-09-23; no `.github/workflows` configuration exists to attach a GitHub Actions run. The complete native throughput sample fails the project's 1 ms submission target. Target-GPU visual acceptance remains unverified.
...
9. Unity, Cinemachine 3.1.0 manual, [*Orbital Follow*](https://docs.unity3d.com/Packages/com.unity.cinemachine@3.1/manual/CinemachineOrbitalFollow.html). Cinemachine 3.1.0 was released 2024-04-01 [12]; the manual page has no separate update date. Accessed 2026-09-23. Applicability: documents independent position damping and the tradeoff between per-axis angular damping and quaternion damping. Limit: Unity component behavior is a design reference, not a dependency or tuning authority for this Rust camera.
10. Unity, Cinemachine 3.1.0 manual, [*Deoccluder*](https://docs.unity3d.com/Packages/com.unity.cinemachine@3.1/manual/CinemachineDeoccluder.html). Cinemachine 3.1.0 was released 2024-04-01 [12]; the manual page has no separate update date. Accessed 2026-09-23. Applicability: documents camera radius and distinct obstruction/return damping. Limit: its collider-based obstacle checks differ from this project's conservative height-field sweep.
11. Unity, Cinemachine 3.1.0 manual, [*Position Composer*](https://docs.unity3d.com/Packages/com.unity.cinemachine@3.1/manual/CinemachinePositionComposer.html). Cinemachine 3.1.0 was released 2024-04-01 [12]; the manual page has no separate update date. Accessed 2026-09-23. Applicability: documents velocity lookahead and per-axis positional damping. Limit: warns that noisy target motion can amplify into camera jitter; the project caps its lookahead at 3 m and 35 ms.
12. Unity, [*Cinemachine 3.1 changelog*](https://docs.unity.cn/Packages/com.unity.cinemachine@3.1/changelog/CHANGELOG.html) and [Unity 6.0 package catalog](https://docs.unity.com/en-us/engine/6000.0/manual/packages-list/packages-all/pack-safe/com-unity-cinemachine). The changelog dates Cinemachine 3.1.0 to 2024-04-01; the catalog page, accessed 2026-09-23, lists 3.1.7 as released for Unity Editor 6000.0 and 3.1.8-pre.2 as available. Applicability: establishes the manual version and package availability used for references 9–11. Limit: package availability does not validate this game's implementation or tuning.
